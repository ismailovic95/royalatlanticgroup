...
class Dsm_Supreme_Modules_Pro_For_Divi {
	...
	private function load_dependencies() {
		 ...
		 * side of the site.
		 */
		include plugin_dir_path( dirname( __FILE__ ) ) . 'includes/license/class.sl.licence.php';
		include plugin_dir_path( dirname( __FILE__ ) ) . 'includes/license/class.sl.plugin-updater.php';
	...
	private function define_admin_hooks() {
		...
		// add_filter( 'auto_update_plugin', array( $this, 'dsm_auto_update_plugins' ), 10, 2 );
		...
		add_action( 'init', array( $this, 'dsm_attachment_category_fields' ), 1 );
		add_action( 'wp_ajax_divi_supreme_get_attachments', array( $this, 'divi_supreme_get_attachments' ) );
		...
		if ( class_exists( 'WPCF7' ) ) {
			...
			remove_action( 'wpcf7_init', 'wpcf7_add_form_tag_date', 20 );
			add_action( 'wpcf7_init', array( $this, 'dsm_wpcf7_add_form_tag_date' ) );

			add_action( 'wpcf7_after_update', array( $this, 'dsm_delete_cf7_library_transient' ), 10, 3 );
			add_action( 'wpcf7_after_create', array( $this, 'dsm_delete_cf7_library_transient' ), 10, 3 );
			add_action( 'wpcf7_after_save', array( $this, 'dsm_delete_cf7_library_transient' ), 10, 3 );
			add_action( 'before_delete_post', array( $this, 'dsm_delete_cf7_library' ), 10, 3 );
		}
	...
	public function add_async_defer_attribute( $tag, $handle ) {
		...
		$twitter       = array( 'dsm-twitter-embed' );

		if ( isset( get_option( 'dsm_settings_misc' )['dsm_defer_js_assets'] ) && 'on' === get_option( 'dsm_settings_misc' )['dsm_defer_js_assets'] ) {
			...
			$defer_scripts = array(
				...
				'dsm-notation',
				'dsm-facebook',
				'dsm-magnific-popup-image',
				'dsm-magnific-popup-video',
				'dsm-imagesloaded',
				'dsm-masonry',
				// Modules.
				'dsm-image-hotspots',
				...
				'dsm-text-rotator',
				'dsm-shuffle-letters',
				'dsm-before-after-image',
				'dsm-lottie-module',
				'dsm-block-reveal-text',
				'dsm-block-reveal-image',
				'dsm-card-carousel',
				'dsm-image-carousel',
				'dsm-tilt-image',
				'dsm-image-text-reveal',
				'dsm-image-hover-reveal',
				'dsm-contact-form-7',
				'dsm-caldera-forms',
				'dsm-text-notation',
				'dsm-flipbox-click',
		...
		if ( in_array( $handle, $twitter, true ) ) {
			return str_replace( ' src', ' async crossorigin="anonymous" charset="utf-8" src', $tag );
		}
	...
	/**
	 * Message for Auto-updates.
	 *
	 * @since 4.8.85
	 */
	public function dsm_auto_update_setting_html( $html, $plugin_file, $plugin_data ) {
		if ( 'supreme-modules-pro-for-divi/supreme-modules-pro-for-divi.php' === $plugin_file ) {
			$html = __( 'Auto-updates are not available for this plugin. This plugin new update will only show up on your WordPress updates.', 'dsm-supreme-modules-pro-for-divi' );
		}
	...
	/**
	 * True for Auto-updates.
	 *
	 * @since 4.9.52
	 */
	public function dsm_auto_update_plugins( $update, $item ) {
		$plugins = array( 'supreme-modules-pro-for-divi' );
		// For example, 'supreme-modules-pro-for-divi/supreme-modules-pro-for-divi.php' becomes 'supreme-modules-pro-for-divi'.
		if ( empty( $item->slug ) ) {
			$item->slug = basename( plugin_dir_url( __DIR__ ) . 'supreme-modules-pro-for-divi', '.php' );
		}
	...
	public function output_modules_popup( $output, $render_slug, $module ) {
		if ( $this->dsm_load_divi_modules() !== $render_slug ) {
			...
			if ( isset( $module->props['dsm_modules_popup'] ) && 'on' === $module->props['dsm_modules_popup'] ) {
				...
				$dsm_modules_popup_cookie_expiry          = 'onload' === $module->props['dsm_modules_popup_trigger'] || 'scroll' === $module->props['dsm_modules_popup_trigger'] || 'exit' === $module->props['dsm_modules_popup_trigger'] ? ' data-dsm-popup-cookie-expiry="' . $module->props['dsm_modules_popup_cookie_days'] . '"' : '';
				...
				$output = str_replace( 'class="dsm-content-timeline-content-wrapper ', $data . ' class="dsm-content-timeline-content-wrapper dsm-popup-' . $dsm_modules_popup_trigger . $dsm_modules_popup_check_trigger . ' ', $output );
				...
				if ( 'onload' === $dsm_modules_popup_trigger || 'scroll' === $dsm_modules_popup_trigger || 'exit' === $dsm_modules_popup_trigger ) {
	...
	public function dsm_wpcf7_select_form_tag_handler( $tag ) {
		...
		$html = sprintf(
			'<span class="wpcf7-form-control-wrap dsm-contact-form-7-select" data-name="%1$s"><select %2$s>%3$s</select>%4$s</span>',
			esc_attr( $tag->name ),
	...
	public function dsm_wpcf7_add_form_tag_date() {
		wpcf7_add_form_tag(
			array( 'date', 'date*' ),
			'dsm_wpcf7_date_form_tag_handler',
			array(
				'name-attr' => true,
			)
		);
	...
	public function dsm_wpcf7_date_form_tag_handler( $tag ) {
		if ( empty( $tag->name ) ) {
			return '';
		}
		...
		$atts['class']    = $tag->get_class_option( $class . ' dsm_contact_form_7_date input' );
		$atts['id']       = $tag->get_id_option();
		$atts['tabindex'] = $tag->get_option( 'tabindex', 'signed_int', true );
		...
		$html = sprintf(
			'<span class="wpcf7-form-control-wrap dsm-contact-form-7-date" data-name="%1$s"><input %2$s />%3$s</span>',
			esc_attr( $tag->name ),
			wpcf7_format_atts( $atts ),
			$validation_error
		);
	...
	public static function dsm_delete_cf7_library( $postid ) {
	...
	/**
	 * Add Custom Taxonomy Category to Media
	 *
	 * @since 4.8.88
	 */
	public static function dsm_attachment_category_fields() {
		$labels = array(
		...
		register_taxonomy( 'dsm-attachment-category', array( 'attachment' ), $args );

		global $wp_taxonomies;
		if ( ! taxonomy_exists( 'dsm-attachment-category' ) ) {
			return false;
		}

		$wp_taxonomies['dsm-attachment-category']->update_count_callback = '_update_generic_term_count';
	}

	/**
	 * Get Attachment Categories for Filterable Gallery
	 *
	 * @since 4.8.88
	 */

	public function divi_supreme_get_attachments() {
		$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['nonce'] ) ) : '';

		if ( ! wp_verify_nonce( $nonce, 'dsm_filterable_gallery' ) ) {
			echo wp_json_encode( array() );
			wp_die();
		}
		$ids         = isset( $_REQUEST['ids'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['ids'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification
		$categories  = isset( $_REQUEST['categories'] ) ? array_map( 'sanitize_text_field', wp_unslash( $_REQUEST['categories'] ) ) : array();
		$use_dynamic = isset( $_REQUEST['dynamic'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['dynamic'] ) ) : false; // phpcs:ignore WordPress.Security.NonceVerification
		$max         = isset( $_REQUEST['max'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['max'] ) ) : -1; // phpcs:ignore WordPress.Security.NonceVerification
		$order       = isset( $_REQUEST['order'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['order'] ) ) : 'ASC'; // phpcs:ignore WordPress.Security.NonceVerification
			...
			if ( 'true' !== $use_dynamic ) {
				unset( $attachments_args['order'] );
				$attachments_args['orderby'] = 'post__in';
			}

			if ( 'true' === $use_dynamic ) {
				$attachments_args['numberposts'] = $max;
			}
			...
			foreach ( $_attachments as $key => $val ) {
				...
				foreach ( $available_sizes as $available_size ) {
					$attachment_src_data = wp_get_attachment_image_src( $val->ID, $available_size );

					if ( false !== $attachment_src_data ) {
						$sizes[ $available_size ] = $attachment_src_data[0]; // Only attaching source.
					}
				}

				$attachments[ $key ]->sizes = $sizes;

				$current_categories = get_the_terms( $val->ID, 'dsm-attachment-category' );
				...
				// Attaching categories and caption.
				$attachments[ $key ]->categories  = $attachment_categories;
				$attachments[ $key ]->caption     = wp_get_attachment_caption( $val->ID );
				$attachments[ $key ]->description = $val->post_content;
			}

			echo wp_json_encode( $_attachments );
