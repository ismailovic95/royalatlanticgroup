...
use OctolizeShippingCanadaPostVendor\Octolize\ShippingExtensions\Plugin\PluginFactory;
use OctolizeShippingCanadaPostVendor\Octolize\ShippingExtensions\Plugin\PluginSorter;
 ...
 * .
 */
class Page implements \OctolizeShippingCanadaPostVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
     ...
     */
    public function __construct(string $assets_url, \OctolizeShippingCanadaPostVendor\Octolize\ShippingExtensions\Tracker\ViewPageTracker $view_page_tracker)
    {
    ...
    {
        \add_submenu_page(self::PARENT_SLUG, \_x('Shipping Extensions', 'Page title', 'octolize-canada-post-shipping'), $this->get_menu_title(), 'manage_options', self::MENU_SLUG, [$this, 'render_page']);
    }
    /**
     * @return void
     */
    ...
    {
        \wp_enqueue_style(\OctolizeShippingCanadaPostVendor\Octolize\ShippingExtensions\Assets::HANDLE);
    ...
    {
        $menu_title = \nl2br(\_x("Shipping\nExtensions", 'Menu Title', 'octolize-canada-post-shipping'));
    ...
    {
        $plugins = \OctolizeShippingCanadaPostVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::get_plugins();
        list($plugins_priority, $categories_priority) = $this->get_plugins_and_categorites_priority();
        $sorter = new \OctolizeShippingCanadaPostVendor\Octolize\ShippingExtensions\Plugin\PluginSorter($plugins, $plugins_priority, $categories_priority);
    ...
    private function get_plugins_and_categorites_priority() : array
    {
        ...
        $categories = \OctolizeShippingCanadaPostVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::get_categories();
        $plugins_priority = [];
        $categories_priority = [];
        if (\in_array('flexible-shipping/flexible-shipping.php', $active_plugins, \true)) {
            ...
            $categories_priority[] = $categories[\OctolizeShippingCanadaPostVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::CATEGORY_CUSTOMIZABLE_RATES];
            $categories_priority[] = $categories[\OctolizeShippingCanadaPostVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::CATEGORY_SHIPPING_LABELS];
        }
        if (\in_array('flexible-shipping-ups/flexible-shipping-ups.php', $active_plugins, \true)) {
            ...
            $categories_priority[] = $categories[\OctolizeShippingCanadaPostVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::CATEGORY_LIVE_RATES];
        }
    ...
    {
        return \array_values(\OctolizeShippingCanadaPostVendor\Octolize\ShippingExtensions\Plugin\PluginFactory::get_categories());
    }
}
