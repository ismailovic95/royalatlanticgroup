...
namespace OctolizeShippingCanadaPostVendor\WPDesk\Tracker;

use OctolizeShippingCanadaPostVendor\WPDesk\PluginBuilder\Plugin\Hookable;
use OctolizeShippingCanadaPostVendor\WPDesk\View\Renderer\SimplePhpRenderer;
use OctolizeShippingCanadaPostVendor\WPDesk\View\Resolver\DirResolver;
class OptInPage implements \OctolizeShippingCanadaPostVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    /**
     * @var string
     */
    ...
    {
        \add_submenu_page('', 'WP Desk Tracker', 'WP Desk Tracker', 'manage_options', 'wpdesk_tracker_' . $this->plugin_slug, array($this, 'wpdesk_tracker_page'));
    }
    public function wpdesk_tracker_page()
    {
        ...
        $shop = new \OctolizeShippingCanadaPostVendor\WPDesk\Tracker\Shop($shop_url);
        $terms_url = $shop->get_usage_tracking_page();
        $logo = $shop->get_shop_logo_file();
        $logo_url = \plugin_dir_url(__FILE__) . '../../../assets/images/' . $logo;
        $renderer = new \OctolizeShippingCanadaPostVendor\WPDesk\View\Renderer\SimplePhpRenderer(new \OctolizeShippingCanadaPostVendor\WPDesk\View\Resolver\DirResolver(__DIR__ . '/views'));
        // WPCS: XSS ok.
    ...
    public function admin_init()
    {
        if (isset($_GET['wpdesk_tracker']) && $_GET['wpdesk_tracker'] === $this->plugin_slug) {
            if (isset($_GET['allow']) && isset($_GET['security']) && \wp_verify_nonce($_GET['security'], $this->plugin_slug)) {
                if ($_GET['allow'] === '1') {
                    $persistence = new \OctolizeShippingCanadaPostVendor\WPDesk_Tracker_Persistence_Consent();
