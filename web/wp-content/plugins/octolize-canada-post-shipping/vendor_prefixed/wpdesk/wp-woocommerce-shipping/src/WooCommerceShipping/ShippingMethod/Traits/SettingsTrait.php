...
use OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ApiStatus\ApiStatusSettingsDefinitionDecorator;
use OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomFields\CouldNotFindService;
use OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomFields\Services\FieldServices;
use OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomFields\FieldHandlingFees;
use OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomFields\FieldsFactory;
...
use OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\MethodFieldsFactory;
...
trait SettingsTrait
{
     ...
     */
    protected function get_settings_definition_from_service(\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\PluginShippingDecisions $plugin_shipping_decisions)
    {
        ...
        if ($shipping_service instanceof \OctolizeShippingCanadaPostVendor\WPDesk\AbstractShipping\ShippingServiceCapability\CanTestSettings) {
            $settings_definitions = new \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ApiStatus\ApiStatusSettingsDefinitionDecorator($settings_definitions, $shipping_service->get_field_before_api_status_field(), $plugin_shipping_decisions->get_field_api_status_ajax(), $shipping_service->get_unique_id());
        }
     ...
     * @return array
     */
    private function get_form_fields_from_shipping_service(\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\PluginShippingDecisions $plugin_shipping_decisions)
    {
    ...
    public function generate_settings_html($form_fields = array(), $echo = \true)
    {
        ...
        foreach ($form_fields as $field_id => $values) {
            ...
            if ($options_generator === \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields::OPTIONS_GENERATOR_COUNTRY_STATE) {
                $values['options'] = $this->prepare_country_state_options();
            }
            if ($options_generator === \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomOrigin\InstanceCustomOriginFields::OPTIONS_GENERATOR_COUNTRY_STATE_FOR_ORIGIN) {
            ...
            if (\method_exists($this, 'generate_' . $type . '_html')) {
                ...
                } catch (\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomFields\CouldNotFindService $e) {
                    $custom_field = null;
                }
    ...
    private function prepare_custom_field_types($fields)
    {
        ...
        if ($this instanceof \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasHandlingFees) {
            $fields = $this->replace_handling_fees_field_if_exists($fields);
        }
        if ($this instanceof \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasCustomOrigin) {
            $custom_origin_fields = new \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields($this instanceof \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasInstanceCustomOrigin);
            $fields = $custom_origin_fields->replace_fallback_field_if_exists($fields, $this);
        }
        if ($this instanceof \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasInstanceCustomOrigin) {
            $instance_custom_origin_fields = new \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomOrigin\InstanceCustomOriginFields(\true);
            $fields = $instance_custom_origin_fields->replace_fallback_field_if_exists($fields, $this);
        }
        if ($this instanceof \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasFreeShipping) {
            $free_shipping_fields = new \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\FreeShipping\FreeShippingFields();
    ...
    protected function create_fields_factory()
    {
        if ($this->fields_factory === null) {
            $this->fields_factory = new \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\MethodFieldsFactory();
        }
    ...
    private function replace_fallback_field_if_exists($settings)
    {
        ...
        foreach ($settings as $key => $field) {
            if ($field['type'] === \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\Fallback\FallbackRateMethod::FIELD_TYPE_FALLBACK) {
                $new_settings[\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\Fallback\FallbackRateMethod::FIELD_ENABLE_FALLBACK] = ['title' => \__('Fallback', 'octolize-canada-post-shipping'), 'type' => 'checkbox', 'label' => \__('Enable fallback', 'octolize-canada-post-shipping'), 'description' => \__('Enable to offer flat rate cost for shipping so that the user can still checkout, if API for some reason returns no matching rates.', 'octolize-canada-post-shipping'), 'desc_tip' => \true, 'default' => 'no'];
                $new_settings[\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\Fallback\FallbackRateMethod::FIELD_FALLBACK_COST] = ['title' => \__('Fallback Cost', 'octolize-canada-post-shipping'), 'type' => 'price', 'required' => \true, 'description' => \__('Enter only a numeric value without the currency symbol.', 'octolize-canada-post-shipping'), 'desc_tip' => \true, 'default' => ''];
            } else {
    ...
    private function replace_handling_fees_field_if_exists($settings)
    {
        $new_settings = [];
        foreach ($settings as $key => $field) {
            if ($field['type'] === \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomFields\FieldHandlingFees::FIELD_TYPE) {
                $field_handling_fees = new \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomFields\FieldHandlingFees();
    ...
    {
        foreach ($settings as $key => $field) {
            if (isset($field['type']) && \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomFields\Services\FieldServices::FIELD_TYPE === $field['type']) {
                $settings[$key]['sanitize_callback'] = [\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomFields\Services\FieldServices::class, 'sanitize'];
            }
        }
        return $settings;
    }
    ...
    public function admin_options()
    {
        ...
        /** @TODO: move to custom field & field footer. */
        if ($this instanceof \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasHandlingFees) {
            $price_adjustment_type_field = $settings_prefix . '_' . \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomFields\FieldHandlingFees::OPTION_PRICE_ADJUSTMENT_TYPE;
            $price_adjustment_value_field = $settings_prefix . '_' . \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomFields\FieldHandlingFees::OPTION_PRICE_ADJUSTMENT_VALUE;
            $price_adjustment_type_none = \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\HandlingFees\PriceAdjustmentNone::ADJUSTMENT_TYPE;
    ...
    {
        return new \OctolizeShippingCanadaPostVendor\WPDesk\AbstractShipping\Settings\SettingsValuesAsArray(\array_merge($this->settings, $this->instance_id ? $this->instance_settings : []));
    }
}
