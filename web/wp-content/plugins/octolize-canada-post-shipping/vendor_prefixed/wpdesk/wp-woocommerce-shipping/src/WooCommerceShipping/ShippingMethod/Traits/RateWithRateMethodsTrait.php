...
trait RateWithRateMethodsTrait
{
    ...
    {
        $origin_country = \explode(':', $this->get_option(\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields::ORIGIN_COUNTRY, ''));
        return new \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingBuilder\CustomOriginAddressSender($this->get_option(\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields::ORIGIN_ADDRESS, ''), '', $this->get_option(\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields::ORIGIN_CITY, ''), $this->get_option(\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields::ORIGIN_POSTCODE, ''), isset($origin_country[0]) ? $origin_country[0] : '', isset($origin_country[1]) ? $origin_country[1] : '');
    }
    /**
     * @return CustomOriginAddressSender .
     */
    ...
    {
        $origin_country = \explode(':', $this->get_instance_option(\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomOrigin\InstanceCustomOriginFields::ORIGIN_COUNTRY, ''));
        return new \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingBuilder\CustomOriginAddressSender($this->get_instance_option(\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomOrigin\InstanceCustomOriginFields::ORIGIN_ADDRESS, ''), '', $this->get_instance_option(\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomOrigin\InstanceCustomOriginFields::ORIGIN_CITY, ''), $this->get_instance_option(\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomOrigin\InstanceCustomOriginFields::ORIGIN_POSTCODE, ''), isset($origin_country[0]) ? $origin_country[0] : '', isset($origin_country[1]) ? $origin_country[1] : '');
    }
    /**
     * Is custom origin?
     *
    ...
    {
        return $this instanceof \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasCustomOrigin && 'yes' === $this->get_option(\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields::CUSTOM_ORIGIN, 'no');
    }
    /**
    ...
    {
        return $this instanceof \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasInstanceCustomOrigin && 'yes' === $this->get_instance_option(\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomOrigin\InstanceCustomOriginFields::CUSTOM_ORIGIN, 'no');
    }
    /**
    ...
    public function get_origin_country_state()
    {
        if ($this->is_custom_origin()) {
            return $this->get_option(\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields::ORIGIN_COUNTRY, '');
        } else {
    ...
    {
        if ($this instanceof \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasInstanceCustomOrigin && $this->is_custom_origin_for_instance()) {
            return $this->create_sender_address_from_custom_origin_from_instance_options();
        }
        if ($this instanceof \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasCustomOrigin && $this->is_custom_origin()) {
            return $this->create_sender_address_from_custom_origin();
        }
        return new \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceAddressSender();
    }
    /**
    ...
    protected function create_shipping_builder(array $package)
    {
        if ($this->shipping_builder === null) {
            $this->shipping_builder = new \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingBuilder();
        }
        ...
        $this->shipping_builder->set_receiver_address(new \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceAddressReceiver($package));
     ...
     */
    private function add_rate_method(\OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\RateMethod $method)
    {
     ...
     */
    private function handle_rating_using_methods(\OctolizeShippingCanadaPostVendor\Psr\Log\LoggerInterface $logger, \OctolizeShippingCanadaPostVendor\WPDesk\AbstractShipping\ShippingService $service, array $package, \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingMetaDataBuilder $metadata_builder)
    {
        $loggerWithErrorCatching = new \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\ErrorLogCatcher($logger);
    ...
    public function add_rate($args = array())
    {
        ...
        if ($this instanceof \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasEstimatedDeliveryDates) {
        ...
        if ($cart instanceof \WC_Cart && $this instanceof \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasFreeShipping) {
            $free_shipping = new \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\FreeShipping\FreeShipping($this, $logger);
