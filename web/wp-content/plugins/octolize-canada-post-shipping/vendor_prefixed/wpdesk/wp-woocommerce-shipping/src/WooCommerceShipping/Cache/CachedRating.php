...
namespace OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\Cache;
...
class CachedRating
{
     ...
     */
    public function __construct($shop_settings_md5_hash, \OctolizeShippingCanadaPostVendor\WPDesk\Persistence\PersistentContainer $container)
    {
     ...
     */
    public function rate_shipment(\OctolizeShippingCanadaPostVendor\WPDesk\AbstractShipping\Settings\SettingsValuesAsArray $settings, \OctolizeShippingCanadaPostVendor\WPDesk\AbstractShipping\Shipment\Shipment $shipment, \OctolizeShippingCanadaPostVendor\WPDesk\AbstractShipping\ShippingServiceCapability\CanRate $service)
    {
     ...
     * @return ShipmentRating
     */
    public function rate_shipment_to_collection_point(\OctolizeShippingCanadaPostVendor\WPDesk\AbstractShipping\Settings\SettingsValuesAsArray $settings, \OctolizeShippingCanadaPostVendor\WPDesk\AbstractShipping\Shipment\Shipment $shipment, \OctolizeShippingCanadaPostVendor\WPDesk\AbstractShipping\CollectionPoints\CollectionPoint $collection_point, \OctolizeShippingCanadaPostVendor\WPDesk\AbstractShipping\ShippingServiceCapability\CanRateToCollectionPoint $service)
    {
     ...
     * @return string
     */
    private function prepare_rates_key(\OctolizeShippingCanadaPostVendor\WPDesk\AbstractShipping\Settings\SettingsValuesAsArray $settings, \OctolizeShippingCanadaPostVendor\WPDesk\AbstractShipping\Shipment\Shipment $shipment, string $collection_point = '')
    {
     ...
     * @return string
     */
    private function prepare_shipment_md5_hash(\OctolizeShippingCanadaPostVendor\WPDesk\AbstractShipping\Shipment\Shipment $shipment)
    {
     ...
     * @return void
     */
    private function store_shipment_rating_in_cache($rates_key, \OctolizeShippingCanadaPostVendor\WPDesk\AbstractShipping\Rate\ShipmentRating $shipment_rating)
    {
        $rates = [];
                ...
                if (isset($data[self::RATES]) && \is_array($data[self::RATES])) {
                    foreach ($data[self::RATES] as $rate) {
                        if (\is_array($rate)) {
                            ...
                            $total_charge = new \OctolizeShippingCanadaPostVendor\WPDesk\AbstractShipping\Rate\Money();
