...
use OctolizeShippingCanadaPostVendor\WPDesk\RepositoryRating\DisplayStrategy\DisplayDecision;
/**
 * Can display text petition.
 */
class TextPetitionDisplayer implements \OctolizeShippingCanadaPostVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
     ...
     */
    public function __construct(string $display_on_action, \OctolizeShippingCanadaPostVendor\WPDesk\RepositoryRating\DisplayStrategy\DisplayDecision $display_decision, \OctolizeShippingCanadaPostVendor\WPDesk\RepositoryRating\PetitionText $petition_text_generator)
    {
    ...
    public function enqueue_css_if_should()
    {
        if ($this->display_decision->should_display()) {
            \wp_enqueue_style('octolize-canada-post-shipping', \plugin_dir_url(__DIR__ . '/../assets/css/style.css') . 'style.css', array(), self::SCRIPTS_VERSION);
        }
    }
    /**
     * @internal
