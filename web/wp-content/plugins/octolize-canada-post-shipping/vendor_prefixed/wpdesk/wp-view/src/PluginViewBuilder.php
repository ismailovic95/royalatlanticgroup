...
namespace OctolizeShippingCanadaPostVendor\WPDesk\View;

use OctolizeShippingCanadaPostVendor\WPDesk\View\Renderer\SimplePhpRenderer;
use OctolizeShippingCanadaPostVendor\WPDesk\View\Resolver\ChainResolver;
use OctolizeShippingCanadaPostVendor\WPDesk\View\Resolver\DirResolver;
use OctolizeShippingCanadaPostVendor\WPDesk\View\Resolver\WPThemeResolver;
/**
...
class PluginViewBuilder
{
    ...
    {
        $resolver = new \OctolizeShippingCanadaPostVendor\WPDesk\View\Resolver\ChainResolver();
        $resolver->appendResolver(new \OctolizeShippingCanadaPostVendor\WPDesk\View\Resolver\WPThemeResolver(\basename($this->plugin_dir)));
        foreach ($this->template_dirs as $dir) {
            $dir = \trailingslashit($this->plugin_dir) . \trailingslashit($dir);
            $resolver->appendResolver(new \OctolizeShippingCanadaPostVendor\WPDesk\View\Resolver\DirResolver($dir));
        }
        return new \OctolizeShippingCanadaPostVendor\WPDesk\View\Renderer\SimplePhpRenderer($resolver);
    }
    /**
