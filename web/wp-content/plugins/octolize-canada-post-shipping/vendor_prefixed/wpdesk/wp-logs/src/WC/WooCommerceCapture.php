...
use OctolizeShippingCanadaPostVendor\WPDesk\Logger\WC\Exception\WCLoggerAlreadyCaptured;
/**
...
class WooCommerceCapture
{
    ...
    public function __construct(\OctolizeShippingCanadaPostVendor\Monolog\Logger $monolog)
    {
    ...
    private function prepareCaptureHookCallable()
    {
        ...
        if ($this->captureHookFunction === null) {
            $this->captureHookFunction = function () use($monolog) {
                return new \OctolizeShippingCanadaPostVendor\WPDesk\Logger\WC\WooCommerceMonologPlugin($monolog, $this->originalWCLogger);
            };
            $this->monolog->pushHandler(new \OctolizeShippingCanadaPostVendor\WPDesk\Logger\WC\WooCommerceHandler($this->originalWCLogger));
        }
    }
    /**
    ...
    public function captureWcLogger()
    {
        if (self::isSupportedWCVersion()) {
            if ($this->isCaptured) {
                throw new \OctolizeShippingCanadaPostVendor\WPDesk\Logger\WC\Exception\WCLoggerAlreadyCaptured('Try to free wc logger first.');
            }
