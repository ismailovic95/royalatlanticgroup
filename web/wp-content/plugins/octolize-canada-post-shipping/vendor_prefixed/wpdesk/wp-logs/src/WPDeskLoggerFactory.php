...
use OctolizeShippingCanadaPostVendor\Monolog\Handler\StreamHandler;
use OctolizeShippingCanadaPostVendor\Psr\Log\LogLevel;
use OctolizeShippingCanadaPostVendor\WPDesk\Logger\WC\WooCommerceCapture;
use OctolizeShippingCanadaPostVendor\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \OctolizeShippingCanadaPostVendor\WPDesk\Logger\BasicLoggerFactory
{
    const DEFAULT_LOGGER_CHANNEL_NAME = 'wpdesk';
    /** @var string Log to file when level is */
    const LEVEL_WPDESK_FILE = \OctolizeShippingCanadaPostVendor\Psr\Log\LogLevel::DEBUG;
    /** @var string Log to wc logger when level is */
    const LEVEL_WC = \OctolizeShippingCanadaPostVendor\Psr\Log\LogLevel::ERROR;
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\OctolizeShippingCanadaPostVendor\Monolog\Registry::hasLogger($name)) {
            \OctolizeShippingCanadaPostVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\OctolizeShippingCanadaPostVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \OctolizeShippingCanadaPostVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \OctolizeShippingCanadaPostVendor\Monolog\Logger($name);
        }
        if (\OctolizeShippingCanadaPostVendor\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \OctolizeShippingCanadaPostVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\OctolizeShippingCanadaPostVendor\Monolog\Logger $logger)
    {
        if (!\defined('OctolizeShippingCanadaPostVendor\\WC_LOG_THRESHOLD')) {
            \define('OctolizeShippingCanadaPostVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \OctolizeShippingCanadaPostVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \OctolizeShippingCanadaPostVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \OctolizeShippingCanadaPostVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \OctolizeShippingCanadaPostVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\OctolizeShippingCanadaPostVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \OctolizeShippingCanadaPostVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \OctolizeShippingCanadaPostVendor\Monolog\Registry::hasLogger($name);
    }
}
