...
trait MessageTrait
{
    ...
    {
        if (!$this->stream) {
            $this->stream = \OctolizeShippingCanadaPostVendor\GuzzleHttp\Psr7\Utils::streamFor('');
        }
        return $this->stream;
    }
    public function withBody(\OctolizeShippingCanadaPostVendor\Psr\Http\Message\StreamInterface $body)
    {
        if ($body === $this->stream) {
            return $this;
        }
