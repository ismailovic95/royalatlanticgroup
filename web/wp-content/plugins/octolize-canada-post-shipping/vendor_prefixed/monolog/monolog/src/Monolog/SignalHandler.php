...
class SignalHandler
{
     ...
     */
    public function registerSignalHandler(int $signo, $level = \OctolizeShippingCanadaPostVendor\Psr\Log\LogLevel::CRITICAL, bool $callPrevious = \true, bool $restartSyscalls = \true, ?bool $async = \true) : self
    {
        if (!\extension_loaded('pcntl') || !\function_exists('pcntl_signal')) {
            return $this;
        }
        $level = \OctolizeShippingCanadaPostVendor\Monolog\Logger::toMonologLevel($level);
        if ($callPrevious) {
    ...
    public function handleSignal(int $signo, $siginfo = null) : void
    {
        ...
        $level = $this->signalLevelMap[$signo] ?? \OctolizeShippingCanadaPostVendor\Psr\Log\LogLevel::CRITICAL;
