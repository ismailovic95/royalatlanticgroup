...
use OctolizeShippingCanadaPostVendor\PhpConsole\Connector;
use OctolizeShippingCanadaPostVendor\PhpConsole\Handler as VendorPhpConsoleHandler;
use OctolizeShippingCanadaPostVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \OctolizeShippingCanadaPostVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(array $options = [], ?\OctolizeShippingCanadaPostVendor\PhpConsole\Connector $connector = null, $level = \OctolizeShippingCanadaPostVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\class_exists('OctolizeShippingCanadaPostVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(?\OctolizeShippingCanadaPostVendor\PhpConsole\Connector $connector = null) : \OctolizeShippingCanadaPostVendor\PhpConsole\Connector
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \OctolizeShippingCanadaPostVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \OctolizeShippingCanadaPostVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\OctolizeShippingCanadaPostVendor\PhpConsole\Helper::isRegistered()) {
            \OctolizeShippingCanadaPostVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \OctolizeShippingCanadaPostVendor\PhpConsole\Handler::getInstance();
    ...
    public function getConnector() : \OctolizeShippingCanadaPostVendor\PhpConsole\Connector
    {
    ...
    {
        if ($record['level'] < \OctolizeShippingCanadaPostVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record) : void
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \OctolizeShippingCanadaPostVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \OctolizeShippingCanadaPostVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
