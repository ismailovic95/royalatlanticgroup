 ...
 */
class SlackHandler extends \OctolizeShippingCanadaPostVendor\Monolog\Handler\SocketHandler
{
    /**
     * Slack API token
     * @var string
     ...
     */
    public function __construct(string $token, string $channel, ?string $username = null, bool $useAttachment = \true, ?string $iconEmoji = null, $level = \OctolizeShippingCanadaPostVendor\Monolog\Logger::CRITICAL, bool $bubble = \true, bool $useShortAttachment = \false, bool $includeContextAndExtra = \false, array $excludeFields = array(), bool $persistent = \false, float $timeout = 0.0, float $writingTimeout = 10.0, ?float $connectionTimeout = null, ?int $chunkSize = null)
    {
        if (!\extension_loaded('openssl')) {
            throw new \OctolizeShippingCanadaPostVendor\Monolog\Handler\MissingExtensionException('The OpenSSL PHP extension is required to use the SlackHandler');
        }
    ...
    protected function prepareContentData(array $record) : array
    {
        ...
        if (!empty($dataArray['attachments'])) {
            $dataArray['attachments'] = \OctolizeShippingCanadaPostVendor\Monolog\Utils::jsonEncode($dataArray['attachments']);
        }
        return $dataArray;
    }
    /**
