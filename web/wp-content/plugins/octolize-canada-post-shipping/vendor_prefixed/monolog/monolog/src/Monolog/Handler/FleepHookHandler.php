 ...
 */
class FleepHookHandler extends \OctolizeShippingCanadaPostVendor\Monolog\Handler\SocketHandler
{
     ...
     */
    public function __construct(string $token, $level = \OctolizeShippingCanadaPostVendor\Monolog\Logger::DEBUG, bool $bubble = \true, bool $persistent = \false, float $timeout = 0.0, float $writingTimeout = 10.0, ?float $connectionTimeout = null, ?int $chunkSize = null)
    {
        if (!\extension_loaded('openssl')) {
            throw new \OctolizeShippingCanadaPostVendor\Monolog\Handler\MissingExtensionException('The OpenSSL PHP extension is required to use the FleepHookHandler');
        }
        $this->token = $token;
    ...
    {
        return new \OctolizeShippingCanadaPostVendor\Monolog\Formatter\LineFormatter(null, null, \true, \true);
    }
    /**
