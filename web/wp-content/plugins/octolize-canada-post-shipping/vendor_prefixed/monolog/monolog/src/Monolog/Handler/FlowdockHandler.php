...
use OctolizeShippingCanadaPostVendor\Monolog\Formatter\FlowdockFormatter;
 ...
 */
class FlowdockHandler extends \OctolizeShippingCanadaPostVendor\Monolog\Handler\SocketHandler
{
    /**
     * @var string
     */
     ...
     */
    public function __construct(string $apiToken, $level = \OctolizeShippingCanadaPostVendor\Monolog\Logger::DEBUG, bool $bubble = \true, bool $persistent = \false, float $timeout = 0.0, float $writingTimeout = 10.0, ?float $connectionTimeout = null, ?int $chunkSize = null)
    {
        if (!\extension_loaded('openssl')) {
            throw new \OctolizeShippingCanadaPostVendor\Monolog\Handler\MissingExtensionException('The OpenSSL PHP extension is required to use the FlowdockHandler');
        }
    ...
    {
        if (!$formatter instanceof \OctolizeShippingCanadaPostVendor\Monolog\Formatter\FlowdockFormatter) {
    ...
    {
        return \OctolizeShippingCanadaPostVendor\Monolog\Utils::jsonEncode($record['formatted']['flowdock']);
    }
    /**
