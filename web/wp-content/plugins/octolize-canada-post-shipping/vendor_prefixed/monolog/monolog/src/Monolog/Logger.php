...
use OctolizeShippingCanadaPostVendor\Psr\Log\InvalidArgumentException;
 ...
 */
class Logger implements \OctolizeShippingCanadaPostVendor\Psr\Log\LoggerInterface, \OctolizeShippingCanadaPostVendor\Monolog\ResettableInterface
{
    /**
     ...
     */
    public function pushHandler(\OctolizeShippingCanadaPostVendor\Monolog\Handler\HandlerInterface $handler) : self
    {
     ...
     */
    public function popHandler() : \OctolizeShippingCanadaPostVendor\Monolog\Handler\HandlerInterface
    {
        if (!$this->handlers) {
     ...
     */
    public function addRecord(int $level, string $message, array $context = [], \OctolizeShippingCanadaPostVendor\Monolog\DateTimeImmutable $datetime = null) : bool
    {
        ...
        try {
            ...
            foreach ($this->handlers as $handler) {
                if (null === $record) {
                    ...
                    $record = ['message' => $message, 'context' => $context, 'level' => $level, 'level_name' => $levelName, 'channel' => $this->name, 'datetime' => $datetime ?? new \OctolizeShippingCanadaPostVendor\Monolog\DateTimeImmutable($this->microsecondTimestamps, $this->timezone), 'extra' => []];
                    try {
    ...
    {
        foreach ($this->handlers as $handler) {
            if ($handler instanceof \OctolizeShippingCanadaPostVendor\Monolog\ResettableInterface) {
                $handler->reset();
            }
        }
        foreach ($this->processors as $processor) {
            if ($processor instanceof \OctolizeShippingCanadaPostVendor\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
    ...
    public static function getLevelName(int $level) : string
    {
        if (!isset(static::$levels[$level])) {
            throw new \OctolizeShippingCanadaPostVendor\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels)));
        }
    ...
    public static function toMonologLevel($level) : int
    {
        if (\is_string($level)) {
            ...
            throw new \OctolizeShippingCanadaPostVendor\Psr\Log\InvalidArgumentException('Level "' . $level . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels) + static::$levels));
        }
        if (!\is_int($level)) {
            throw new \OctolizeShippingCanadaPostVendor\Psr\Log\InvalidArgumentException('Level "' . \var_export($level, \true) . '" is not defined, use one of: ' . \implode(', ', \array_keys(static::$levels) + static::$levels));
        }
        return $level;
    }
    /**
