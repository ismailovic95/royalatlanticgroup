...
use OctolizeShippingCanadaPostVendor\Monolog\Handler\SyslogUdp\UdpSocket;
 ...
 */
class SyslogUdpHandler extends \OctolizeShippingCanadaPostVendor\Monolog\Handler\AbstractSyslogHandler
{
    const RFC3164 = 0;
    const RFC5424 = 1;
    const RFC5424e = 2;
     ...
     */
    public function __construct(string $host, int $port = 514, $facility = \LOG_USER, $level = \OctolizeShippingCanadaPostVendor\Monolog\Logger::DEBUG, bool $bubble = \true, string $ident = 'php', int $rfc = self::RFC5424)
    {
        if (!\extension_loaded('sockets')) {
            throw new \OctolizeShippingCanadaPostVendor\Monolog\Handler\MissingExtensionException('The sockets extension is required to use the SyslogUdpHandler');
        }
        parent::__construct($facility, $level, $bubble);
        $this->ident = $ident;
        $this->rfc = $rfc;
        $this->socket = new \OctolizeShippingCanadaPostVendor\Monolog\Handler\SyslogUdp\UdpSocket($host, $port);
    }
    ...
    private function splitMessageIntoLines($message) : array
    {
        ...
        if (\false === $lines) {
            ...
            throw new \RuntimeException('Could not preg_split: ' . $pcreErrorCode . ' / ' . \OctolizeShippingCanadaPostVendor\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $lines;
    }
    /**
     ...
     */
    public function setSocket(\OctolizeShippingCanadaPostVendor\Monolog\Handler\SyslogUdp\UdpSocket $socket) : self
    {
        $this->socket = $socket;
        return $this;
    }
