...
use OctolizeShippingCanadaPostVendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy;
use OctolizeShippingCanadaPostVendor\Monolog\Handler\FingersCrossed\ActivationStrategyInterface;
use OctolizeShippingCanadaPostVendor\Monolog\Logger;
use OctolizeShippingCanadaPostVendor\Monolog\ResettableInterface;
 ...
 */
class FingersCrossedHandler extends \OctolizeShippingCanadaPostVendor\Monolog\Handler\Handler implements \OctolizeShippingCanadaPostVendor\Monolog\Handler\ProcessableHandlerInterface, \OctolizeShippingCanadaPostVendor\Monolog\ResettableInterface, \OctolizeShippingCanadaPostVendor\Monolog\Handler\FormattableHandlerInterface
{
    ...
    public function __construct($handler, $activationStrategy = null, int $bufferSize = 0, bool $bubble = \true, bool $stopBuffering = \true, $passthruLevel = null)
    {
        if (null === $activationStrategy) {
            $activationStrategy = new \OctolizeShippingCanadaPostVendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy(\OctolizeShippingCanadaPostVendor\Monolog\Logger::WARNING);
        }
        // convert simple int activationStrategy to an object
        if (!$activationStrategy instanceof \OctolizeShippingCanadaPostVendor\Monolog\Handler\FingersCrossed\ActivationStrategyInterface) {
            $activationStrategy = new \OctolizeShippingCanadaPostVendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy($activationStrategy);
        }
        ...
        if ($passthruLevel !== null) {
            $this->passthruLevel = \OctolizeShippingCanadaPostVendor\Monolog\Logger::toMonologLevel($passthruLevel);
        }
        if (!$this->handler instanceof \OctolizeShippingCanadaPostVendor\Monolog\Handler\HandlerInterface && !\is_callable($this->handler)) {
    ...
    public function reset()
    {
        ...
        if ($this->getHandler() instanceof \OctolizeShippingCanadaPostVendor\Monolog\ResettableInterface) {
    ...
    {
        if (!$this->handler instanceof \OctolizeShippingCanadaPostVendor\Monolog\Handler\HandlerInterface) {
    ...
    public function setFormatter(\OctolizeShippingCanadaPostVendor\Monolog\Formatter\FormatterInterface $formatter) : \OctolizeShippingCanadaPostVendor\Monolog\Handler\HandlerInterface
    {
        ...
        if ($handler instanceof \OctolizeShippingCanadaPostVendor\Monolog\Handler\FormattableHandlerInterface) {
