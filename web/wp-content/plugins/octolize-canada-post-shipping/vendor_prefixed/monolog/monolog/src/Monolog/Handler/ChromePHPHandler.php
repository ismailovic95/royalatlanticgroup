...
use OctolizeShippingCanadaPostVendor\Monolog\Formatter\ChromePHPFormatter;
 ...
 */
class ChromePHPHandler extends \OctolizeShippingCanadaPostVendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    {
        return new \OctolizeShippingCanadaPostVendor\Monolog\Formatter\ChromePHPFormatter();
    }
    /**
    ...
    protected function send() : void
    {
        ...
        $json = \OctolizeShippingCanadaPostVendor\Monolog\Utils::jsonEncode(self::$json, \OctolizeShippingCanadaPostVendor\Monolog\Utils::DEFAULT_JSON_FLAGS & ~\JSON_UNESCAPED_UNICODE, \true);
        $data = \base64_encode($json);
        if (\strlen($data) > 3 * 1024) {
            self::$overflowed = \true;
            $record = ['message' => 'Incomplete logs, chrome header size limit reached', 'context' => [], 'level' => \OctolizeShippingCanadaPostVendor\Monolog\Logger::WARNING, 'level_name' => \OctolizeShippingCanadaPostVendor\Monolog\Logger::getLevelName(\OctolizeShippingCanadaPostVendor\Monolog\Logger::WARNING), 'channel' => 'monolog', 'datetime' => new \DateTimeImmutable(), 'extra' => []];
