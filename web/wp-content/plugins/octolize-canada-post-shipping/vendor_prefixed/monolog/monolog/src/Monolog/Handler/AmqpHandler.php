...
use OctolizeShippingCanadaPostVendor\Monolog\Formatter\JsonFormatter;
use OctolizeShippingCanadaPostVendor\PhpAmqpLib\Message\AMQPMessage;
use OctolizeShippingCanadaPostVendor\PhpAmqpLib\Channel\AMQPChannel;
use AMQPExchange;
/**
 * @phpstan-import-type Record from \Monolog\Logger
 */
class AmqpHandler extends \OctolizeShippingCanadaPostVendor\Monolog\Handler\AbstractProcessingHandler
{
    /**
     ...
     */
    public function __construct($exchange, ?string $exchangeName = null, $level = \OctolizeShippingCanadaPostVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if ($exchange instanceof \OctolizeShippingCanadaPostVendor\PhpAmqpLib\Channel\AMQPChannel) {
    ...
    private function createAmqpMessage(string $data) : \OctolizeShippingCanadaPostVendor\PhpAmqpLib\Message\AMQPMessage
    {
        $attributes = ['delivery_mode' => 2, 'content_type' => 'application/json'];
        if ($this->extraAttributes) {
            $attributes = \array_merge($attributes, $this->extraAttributes);
        }
        return new \OctolizeShippingCanadaPostVendor\PhpAmqpLib\Message\AMQPMessage($data, $attributes);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return new \OctolizeShippingCanadaPostVendor\Monolog\Formatter\JsonFormatter(\OctolizeShippingCanadaPostVendor\Monolog\Formatter\JsonFormatter::BATCH_MODE_JSON, \false);
    }
}
