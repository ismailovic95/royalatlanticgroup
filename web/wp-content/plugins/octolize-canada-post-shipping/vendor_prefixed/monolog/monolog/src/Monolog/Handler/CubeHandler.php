 ...
 */
class CubeHandler extends \OctolizeShippingCanadaPostVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $url, $level = \OctolizeShippingCanadaPostVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
    ...
    protected function connectUdp() : void
    {
        if (!\extension_loaded('sockets')) {
            throw new \OctolizeShippingCanadaPostVendor\Monolog\Handler\MissingExtensionException('The sockets extension is required to use udp URLs with the CubeHandler');
        }
    ...
    protected function connectHttp() : void
    {
        if (!\extension_loaded('curl')) {
            throw new \OctolizeShippingCanadaPostVendor\Monolog\Handler\MissingExtensionException('The curl extension is required to use http URLs with the CubeHandler');
        }
    ...
    protected function write(array $record) : void
    {
        ...
        if ($this->scheme === 'http') {
            $this->writeHttp(\OctolizeShippingCanadaPostVendor\Monolog\Utils::jsonEncode($data));
        } else {
            $this->writeUdp(\OctolizeShippingCanadaPostVendor\Monolog\Utils::jsonEncode($data));
        }
    }
    ...
    private function writeHttp(string $data) : void
    {
        ...
        \OctolizeShippingCanadaPostVendor\Monolog\Handler\Curl\Util::execute($this->httpConnection, 5, \false);
    }
}
