 ...
 */
class TelegramBotHandler extends \OctolizeShippingCanadaPostVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $apiKey, string $channel, $level = \OctolizeShippingCanadaPostVendor\Monolog\Logger::DEBUG, bool $bubble = \true, string $parseMode = null, bool $disableWebPagePreview = null, bool $disableNotification = null, bool $splitLongMessages = \false, bool $delayBetweenMessages = \false)
    {
        if (!\extension_loaded('curl')) {
            throw new \OctolizeShippingCanadaPostVendor\Monolog\Handler\MissingExtensionException('The curl extension is needed to use the TelegramBotHandler');
        }
    ...
    protected function sendCurl(string $message) : void
    {
        ...
        $result = \OctolizeShippingCanadaPostVendor\Monolog\Handler\Curl\Util::execute($ch);
        if (!\is_string($result)) {
    ...
    private function handleMessageLength(string $message) : array
    {
        ...
        if (!$this->splitLongMessages && \strlen($message) > self::MAX_MESSAGE_LENGTH) {
            return [\OctolizeShippingCanadaPostVendor\Monolog\Utils::substr($message, 0, self::MAX_MESSAGE_LENGTH - \strlen($truncatedMarker)) . $truncatedMarker];
        }
