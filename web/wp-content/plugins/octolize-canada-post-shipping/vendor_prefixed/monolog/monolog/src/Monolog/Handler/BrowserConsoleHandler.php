 ...
 */
class BrowserConsoleHandler extends \OctolizeShippingCanadaPostVendor\Monolog\Handler\AbstractProcessingHandler
{
    /** @var bool */
    ...
    {
        return new \OctolizeShippingCanadaPostVendor\Monolog\Formatter\LineFormatter('[[%channel%]]{macro: autolabel} [[%level_name%]]{font-weight: bold} %message%');
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return [\OctolizeShippingCanadaPostVendor\Monolog\Logger::DEBUG => 'debug', \OctolizeShippingCanadaPostVendor\Monolog\Logger::INFO => 'info', \OctolizeShippingCanadaPostVendor\Monolog\Logger::NOTICE => 'info', \OctolizeShippingCanadaPostVendor\Monolog\Logger::WARNING => 'warn', \OctolizeShippingCanadaPostVendor\Monolog\Logger::ERROR => 'error', \OctolizeShippingCanadaPostVendor\Monolog\Logger::CRITICAL => 'error', \OctolizeShippingCanadaPostVendor\Monolog\Logger::ALERT => 'error', \OctolizeShippingCanadaPostVendor\Monolog\Logger::EMERGENCY => 'error'][$level] ?? 'log';
    }
    /**
     * @return string[]
     */
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (\array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \OctolizeShippingCanadaPostVendor\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \OctolizeShippingCanadaPostVendor\Monolog\Utils::substr($format, $pos + \strlen($match[0][0]));
        }
    ...
    private static function handleCustomStyles(string $style, string $string) : string
    {
        ...
        if (null === $style) {
            ...
            throw new \RuntimeException('Failed to run preg_replace_callback: ' . $pcreErrorCode . ' / ' . \OctolizeShippingCanadaPostVendor\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $style;
    }
    /**
