...
class WoofilterProWpf extends ModuleWpf {
			...
			foreach ( $taxList as $key => $taxName ) {
				if ( strpos( $attrs, $key ) !== false ) {
					...
					if ( $word ) {
						$join .=
							$wpdb->prepare(
								" INNER JOIN {$wpdb->terms} AS wtbp_terms_%1s  ON (wtbp_terms_%2s.term_id=%3s.term_id) ",
								$taxName,
								$taxName,
								$taxName
							);
	...
	public function checkBeforeFiltersFrontendArgs( $args, $filterSettings = array(), $urlQuery = array() ) {
		...
		if ( ! empty( $vars['wpf_oistock'] ) ) {
			add_filter( 'posts_clauses', array( $this, 'addOrderByStockFirst' ), 98 );
		}
	...
	public function loadProductsFilterPro( $q ) {
		...
		if ( ! empty( $defaults['pr_oistock'] ) || isset( $getGet['wpf_oistock'] ) ) {
			add_filter( 'posts_clauses', array( $this, 'addOrderByStockFirst' ), 99 );
		}
	...
	public function addOrderByStockFirst( $args ) {
		...
		remove_filter( 'posts_clauses', array( $this, 'addOrderByStockFirst' ), 99 );
...
//
	...
	public function getProAttributes( $proAttributes, $settings ) {
		...
		if (!is_null($id) && FrameWpf::_()->getModule('statistics') && FrameWpf::_()->getModule('statistics')->getModel()->isEnableStatistics($id)) {
		...
		foreach ( $keysColorGroup as $key ) {
			...
			if ( '' !== $value ) {
				...
				foreach ( $children as $id ) {
					if ( ! in_array( $id, (array) $excludeIds, true ) ) {
						$excludeIds[] = $id;
					}
				}
			}
					...
					if ($searchNumberLogic) {
						...
						if ( $taxExists ) {
							...
							foreach ($taxs as $taxonomy) {
								$attrIds = array();
								$terms = get_terms( $taxonomy, array( 'taxonomy' => $taxonomy, 'fields' => 'all', 'hide_empty' => true ) );
								if (is_array($terms)) {
									foreach ($terms as $term) {
										$v = (int) $term->name;
